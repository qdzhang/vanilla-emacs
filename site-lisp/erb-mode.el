;;; erb-mode.el --- erb major mode based on mhtml-mode  -*- lexical-binding: t; -*-

;; Copyright (C) 2022 qdzhang

;; Author: qdzhang
;; Keywords: wp, languages
;; Package-Requires: ((emacs "26.1") (ruby-mode "1.0"))
...
;; This major mode includes JavaScript/CSS and Ruby erb template language
;; as submode in html-mode. Add erb support based on the built-in `mhtml-mode'.
...
(defun mhtml--construct-submode (mode &rest args)
  ...
  (let ((captured-locals nil)
    ...
    (with-temp-buffer
      ...
      ;; This has to be set to a value other than the erb-mode
  ...
  (syntax-propertize-rules
   ...
   ("<%"
    (0 (ignore
        ...
        ;; Don't apply in a comment
        (unless (syntax-ppss-context (syntax-ppss))
          (mhtml--syntax-propertize-submode mhtml--erb-submode end)))))
...
the rules from `css-mode'; and code inside a <% %> tag element is
indented using `ruby-mode'."
...
(put 'erb-mode 'flyspell-mode-predicate #'mhtml--flyspell-check-word)

(provide 'erb-mode)

;;; erb-mode.el ends here
